...
def boxplot_ing(data:pd.DataFrame, column:str, y_label:str, filter_column:list, step=0, minx=0, quan=0, filter=1_000_000_000_000) -> None:
    """Функция для визуализации количественных данных

    Args:
        data (pd.DataFrame): датафрейм
        column (str): визуализируемые количественные данные
        y_label (str): подпись оси y, должна по смыслу совпадать с column
        filter_column (list): набор качественных данных, разбивающих column на группы, которые будут визуализироваться
        step (int, optional): шаг оси y, может сам рассчитываться
        minx (int, optional): минимум на графике, может сам рассчитываться
        quan (int, optional): максимум данных column который, нужно визуализировать
        filter (_type_, optional): фильтр для column, нужен чтобы убирать большие выбросы в данных
    """
