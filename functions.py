...
# Оптимизируем код, перепишем три функции сверху в одну вызываемую функцию.
def calc_plot(data, column:str, y_label:str, filter_column:list, step=0, minx=0, quan=0, filter=1_000_000_000_000, action_company="None", company="None"):

    # data - датафрейм
    # action_company - действие над датафреймом, 
        # None - ничего не делать, 
        # only_company - отфильтровать данные только по одной компании, 
        # except_company - исключить компанию из данных
    # company - компания, по которой нужно сделать фильтрацию над данными
    # column - визуализируемые количественные данные
    # y_label - подпись оси y, должна по смыслу совпадать с column
    # filter_column - набор качественных данных, разбивающих column на группы, которые будут визуализироваться
    # step - шаг оси y, может сам рассчитываться
    ...
    # вызовем ошибку если определено действие фильтрации, но не значение для фильтрации
    if action_company != "None" and company == "None":
        raise ValueError("Выбрано действие по фильтрации компаний, но не выбрана компания. Или уберите действие фильтрации или выберете компанию.")
    ...
    # уберем данные о кормах определенной компании, если требуется
    if action_company == "except_company":
        data = data[data["company"]!=company]

    # оставим только одну компанию
